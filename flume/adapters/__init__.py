...
def register_adapter(cls):
    ...
    if cls.name in _ADAPTERS.keys():
        raise FlumeException('"%s" adapter already registered' % cls.name)
...
def get_adapter(name, operation):
    ...
    if name not in _ADAPTERS.keys():
        raise FlumeException('"%s" adapter not registered' % name)

    result = _ADAPTERS[name]

    if getattr(result, operation) == getattr(adapter, operation):
        raise FlumeException('"%s" adapter does not support %s' %
                               (name, operation))
