...
class LinechartTest(unittest.TestCase):
    ...
    def test_can_handle_an_empty(self, mock_render):
        ...
        expect(mock_render.call_args).to.eq(mock.call('linechart', {}))

    @mock.patch('flume.sinks.views.gnuplot.Gnuplot.render')
    def test_can_handle_a_single_series_with_datetime_yvalues(self, mock_render):
        (
            emit(points=[
    ...
    @mock.patch('flume.sinks.views.gnuplot.Gnuplot.render')
    def test_can_handle_multiple_series_with_datetime_yvalues(self, mock_render):
        (
            emit(points=[
                {'time': '2016-01-01T00:00:01.000Z', 'name': 'series1', 'value': 1},
